...
public class MySqlPostDao implements PostDao {
    ...
    private static final String SELECT_ALL_POSTS_BY_USER_ID = "SELECT user.username, user.avatar_path, user_id, post_id, date_uploaded FROM user, post WHERE user_id = ? and user.id = post.user_id";
    ...
    private static final String SELECT_POST_BY_ID = "SELECT user.username, user.avatar_path, user_id, post_id, date_uploaded FROM user, post WHERE post_id = ? AND user.id = post.user_id";
    private static final String COUNT_POSTS = "SELECT COUNT(*) FROM post WHERE user_id = ?";
    ...
    @Override
    public Long countPostsForUserWithId(long userId) {
        return jdbcTemplate.queryForObject(COUNT_POSTS, Long.class, userId);
    }
